    ...
    >
      <div className="container-page">
        ...
        <ol
          ...
          className="mt-6 flex flex-wrap items-center gap-2 text-sm text-muted-foreground font-mono list-none p-0"
        >
          {CHAIN.flatMap((c, i) => {
            const items = [
              <li
                key={c}
                className="rounded-sm border border-border bg-card px-2 py-1 text-foreground"
              >
                {c}
              </li>,
            ];
            if (i < CHAIN.length - 1) {
              items.push(
                <li key={`${c}-sep`} aria-hidden className="text-outline">
                  →
                </li>,
              );
            }
